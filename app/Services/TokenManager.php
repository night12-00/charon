...
class TokenManager
{
  ...
  public function createCompositionToken(User $user): CompositionToken
  {
    ...
    $this->cache->rememberForever(
      "app.composition-tokens.$token->apiToken",
      static fn() => $token->audioToken
    );
  ...
  public function deleteCompositionToken(string $plainTextApiToken): void
  {
    ...
    $audioToken = $this->cache->get(
      "app.composition-tokens.$plainTextApiToken"
    );
  ...
  public function refreshApiToken(string $currentPlainTextToken): NewAccessToken
  {
    $newToken = $this->createToken(
      $this->getUserFromPlainTextToken($currentPlainTextToken)
    );
